...
$decryptedData = $dp->getDecrypted($keyAText, $keyBText, $encryptedData);
...
echo "\n - Encryption - \n";
echo "Encrypted Hex:        0x" . bin2hex($encryptedData) . "\n";
echo "\n - Decryption - \n";
...
echo "Encrypted Block Size: " . strlen($encryptedData) . " bytes\n";
echo "Overhead + Plaintext: " . ($encOverhead + strlen($plainText)) . " bytes\n";
echo "Block size correct:   ";
echo (strlen($encryptedData) === ($encOverhead + strlen($plainText))) ? "Yes" : "No";
echo "\n";
echo "\n<<< END CRYPTO TESTS >>>\n";
?>
